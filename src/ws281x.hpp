...
namespace ws281x
{
	...
	constexpr unsigned long HZ_SPI_NEOPIXEL = 2500000;	// this yields 400ns bit-times on the bus
	...
	class TSPIDriver
	{
		...
		public:
			...
			TSPIDriver(const std::string& spidev, const long long hz_speed);
			~TSPIDriver();
	};
	...
	{
		this->b[0] = 0b00000000;
		this->b[1] = (uint8_t)(0b10010010 | ((v & 0b10000000) >> 1) | ((v & 0b01000000) >> 3) | ((v & 0b00100000) >> 5));
		this->b[2] = (uint8_t)(0b01001001 | ((v & 0b00010000) << 1) | ((v & 0b00001000) >> 1));
		this->b[3] = (uint8_t)(0b00100100 | ((v & 0b00000100) << 5) | ((v & 0b00000010) << 3) | ((v & 0b00000001) << 1));
		return *this;
	}
