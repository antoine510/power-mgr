...
nlohmann::json VenusE::SendRequest(nlohmann::json&& req) {
	...
	if(send(_fd, outBuf.data(), outBuf.size(), 0) <= 0) throw std::runtime_error("Error sending data to battery");
	...
	while(true) {
		...
		if(numRead > 0) break;
		if(errno == EAGAIN) {
			if(std::chrono::steady_clock::now() > deadline) throw std::runtime_error("Timed out");
