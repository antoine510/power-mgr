...
namespace ws281x
{
	...
	TSPIDriver::TSPIDriver(const std::string& spidev, const long long hz_speed) : hz_speed(hz_speed), fd(-1)
	{
		SYSERR(this->fd = ::open(spidev.c_str(), O_RDWR | O_CLOEXEC | O_NOCTTY));
	}
